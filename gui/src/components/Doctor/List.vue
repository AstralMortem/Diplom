...
const {doctors, isPending, size, page, pages, total} = storeToRefs(doctorStore)
...
<template>
    <UButton class="fixed top-20 right-2 md:hidden rounded-full" label="Doctors" variant="outline" size="lg" @click="show = true"/>
    <UCard class="h-full w-max" :class="[
        ...
        <div class="flex flex-col gap-2 h-full min-w-[140px]" ref="listRef">
            <DoctorListItem v-for="doctor in doctors" :key="doctor.doctor_id" :doctor="doctor" />
            <div class="flex justify-center items-center h-min w-full" v-if="doctors.length === 0">
